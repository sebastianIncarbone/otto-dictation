...
public sealed class LlamaEngine : ICorrectionEngine, IDisposable
{
    ...
    // could free context/weights while a ChatAsync call on another thread was
    // still running its native inference through them, which is a native
    // access violation rather than a managed exception. generation only
    // protects LoadAsync-vs-Dispose; it has nothing to do with ChatAsync.
    ...
    /// finishes, publishes <see cref="weights"/>/<see cref="context"/>/
    /// <see cref="executor"/> only if its generation is still current — an
    /// orphaned attempt that finishes after being superseded disposes what it
    /// just built instead of leaking it or racing the retry's own writes.
    /// </summary>
    public Task LoadAsync(CancellationToken cancellationToken = default)
    {
        ...
        return CancelableWork.Run(() =>
        {
            ...
            var loadedContext = loadedWeights.CreateContext(parameters);
            var loadedExecutor = new InteractiveExecutor(loadedContext);
            ...
            var published = generation.TryPublish(
                ...
                publish: () =>
                {
                    ...
                    // did, the previously published handles get disposed here
